...
public class JSONArray implements Iterable<Object> {
    ...
    /*
     * If not zero, write a maximum of array elements regardless of indentation for compactness.
     */
    private int newlineAfterElements;
    ...
    /**
     * If set to 1 or more, write a maximum of array elements regardless of indentation for compactness.
     * 
	 * @param newlineAfterElements the newlineAfterElements to set
	 */
	public void setNewlineAfterElements(int newlineAfterElements) {
		this.newlineAfterElements = newlineAfterElements;
	}
            ...
            if (length == 1) {
                ...
                if (newlineAfterElements < 1) {
                    ...
                    for (int i = 0; i < length; i += 1) {
	                    ...
	                    try {
	                        JSONObject.writeValue(writer, this.myArrayList.get(i), 0, 0);
	                    } catch (Exception e) {
	                        throw new JSONException("Unable to write JSONArray value at index: " + i, e);
	                    }
	                    compElemCount++;
	                    if (compElemCount > newlineAfterElements - 1 && i < length - 1) {
							compElemCount = 0;
	                        writer.write(',');
		                    if (indentFactor > 0) {
		                        writer.write('\n');
